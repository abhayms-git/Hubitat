...
def isContactwithLux() { return device.getDataValue('manufacturer') in ['_TZE200_pay2byax'] }
...
def isLuxMeter() { return (is2in1() || is3in1() || is4in1() || isRadar() || isHumanPresenceSensorAIR() || isBlackPIRsensor() || isHumanPresenceSensorScene() || isHumanPresenceSensorFall() || isBlackSquareRadar() || isContactwithLux()) }
...
private handleContact( contactPos, isDigital=false ) {
    ...
    if (device.currentState('contact').value == (contactPos ? "open" : "closed")) {
	   if (settings?.logEnable) log.debug "${device.displayName} ignored repeated event"
	}
	else {
	   if (settings?.logEnable) log.debug "${device.displayName} ${map.descriptionText}"
	   sendEvent(map)
	}
}
